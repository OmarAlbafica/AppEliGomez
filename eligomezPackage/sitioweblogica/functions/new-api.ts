...
function julianDayFromFecha(fechaStr: string): number {
...
function fechaDesdeJulianDay(jd: number): string {
  const a = jd + 32044;
...
function julianDayFromSeconds(seconds: number): number {
...
function obtenerDiaSemanaDesdeFecha(fechaStr: string): number {
  const base = julianDayFromFecha(fechaStr);
  if (!base) {
    return 0;
  }
...
function convertirTimestampAFechaString(timestamp: any): string {
  const DEBUG = true; // Activar logging detallado
  const UTC_OFFSET_HOURS = -6; // UTC-6 para Costa Rica
  
  if (!timestamp) {
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] Entrada null/undefined`);
    return 'N/A';
  }

  if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ========================================`);
  if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ENTRADA: tipo=${typeof timestamp}`);
  if (DEBUG) console.log(`[TIMESTAMP_DEBUG] valor completo=${JSON.stringify(timestamp)}`);
  if (DEBUG) console.log(`[TIMESTAMP_DEBUG] toString()="${String(timestamp)}"`);

  // Si es una string en formato YYYY-MM-DD, devolverla como está
  if (typeof timestamp === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(timestamp)) {
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ✅ DETECTADO: String YYYY-MM-DD -> "${timestamp}"`);
    return timestamp;
  }
  ...
  if (typeof timestamp === 'string' && /^\d{4}-\d{2}-\d{2}T/.test(timestamp)) {
    ...
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ✅ DETECTADO: String ISO -> extraer fecha="${resultado}"`);
    return resultado;
  }

  // Si es una string, intentar parsear como fecha española
  if (typeof timestamp === 'string') {
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] 🔍 Intentando parsear como fecha española...`);
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] String recibido: "${timestamp}"`);
    const human = parseFechaHumana(timestamp);
    if (human) {
      if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ✅ CONVERTIDO por parseFechaHumana() -> "${human}"`);
      return human;
    }
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ❌ parseFechaHumana() devolvió null`);
  }
  ...
  if (typeof timestamp === 'number') {
    ...
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] 📊 Detectado número (Unix seconds): ${segundos}`);
  } else if (typeof timestamp?.seconds === 'number') {
    segundos = Math.floor(timestamp.seconds);
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] 📊 Detectado .seconds: ${segundos}`);
  } else if (typeof timestamp?._seconds === 'number') {
    segundos = Math.floor(timestamp._seconds);
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] 📊 Detectado ._seconds: ${segundos}`);
  } else if (typeof timestamp?.toMillis === 'function') {
    const millis = timestamp.toMillis();
    segundos = Math.floor(millis / 1000);
    if (DEBUG) console.log(`[TIMESTAMP_DEBUG] 📊 Detectado .toMillis(): millis=${millis} -> seconds=${segundos}`);
  }
  ...
  if (typeof timestamp?.toDate === 'function') {
    try {
      ...
      if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ✅ CONVERTIDO desde .toDate(): "${isoFromToDate}" -> "${resultado}" (UTC${UTC_OFFSET_HOURS})`);
      return resultado;
    } catch (err) {
      if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ❌ ERROR en .toDate(): ${err}`);
    }
  }

  if (DEBUG) console.log(`[TIMESTAMP_DEBUG] ❌ NO SE PUDO CONVERTIR: tipo=${typeof timestamp}, valor="${String(timestamp).substring(0, 100)}"`);
  console.warn(`[convertirTimestamp] No se pudo convertir: tipo=${typeof timestamp}, valor="${String(timestamp).substring(0, 80)}"`);
  return 'N/A';
}

function parseFechaHumana(text: string | undefined): string | null {
...
function obtenerFechaHoyServidorStr(): string {
 ...
 */
function esBase64(str: string): boolean {
 ...
 */
export function registerOptimizedAPI(app: express.Application) {
  /**
      ...
      try {
        ...
        if (encomendistasDoc.exists) {
          ...
          if (pedidoData?.destino_id && encData?.destinos) {
            const destinoEncontrado = encData.destinos.find((d: any) => d.nombre === pedidoData.destino_id);
            if (destinoEncontrado) {
              ...
              if (destinoEncontrado.horarios && Array.isArray(destinoEncontrado.horarios)) {
                horarioEspecifico = destinoEncontrado.horarios.find((h: any) => 
                  h.hora_inicio === pedidoData.hora_inicio && h.hora_fin === pedidoData.hora_fin
                );
              }
...
app.get("/pedidos", async (req, res) => {
  try {
    ...
    const pedidosProcesados = await Promise.all(
      pedidosCompletos.map(async (pedido: any) => {
...
app.get("/pedidos-por-remunerar", async (req, res) => {
  try {
    ...
    const fechaHoyParam = req.query.fecha_hoy as string | undefined;
    let hoyStr: string;
  ...
  try {
    ...
    if (diaHoy === 3 || diaHoy === 6) {
      ...
      while (true) {
        ...
        if (diasRetrocedidos > 7) break; // Protección infinito
      }
    }
    ...
    const pedidosUrgentes: any[] = [];
    
    for (const doc of pedidosSnapshot.docs) {
      const data = doc.data();
      if (data.estado?.toLowerCase() !== 'pendiente') continue;
...
app.get("/pedidos-para-envios", async (req, res) => {
  try {
    ...
    let pedidosParaEnvios: any[] = [];
    
    for (const doc of pedidosSnapshot.docs) {
      const data = doc.data();
      const estado = data.estado?.toLowerCase();
      
      if (estado !== 'pendiente' && estado !== 'empacada') continue;
 ...
 */
/**
 * CAMBIAR ESTADO DE PEDIDO - SIMPLE (SIN FOTOS)
 * 
 * La foto se maneja ÚNICAMENTE en /subirFotoPaquete
 * Este endpoint SOLO:
 * - Actualiza el estado
 * - Registra auditoría
 * - Guarda cambio en subcolección cambios_estado
 * 
 * POST /pedido/:pedidoId/cambiar-estado
 * Body: { nuevoEstado, notas?, usuario_email?, usuario_id? }
 */
app.post("/pedido/:pedidoId/cambiar-estado", async (req, res) => {
  try {
    const { pedidoId } = req.params;
    const { nuevoEstado, notas, usuario_email, usuario_id } = req.body;
    const db = admin.firestore();
    ...
    const estadoAuditMap: { [key: string]: string } = {
...
app.get("/diagnosticar-fechas", async (req, res) => {
  try {
    ...
    const diasNombreAIndice: { [key: string]: number } = {
      'domingo': 0, 'dom': 0,
      'lunes': 1, 'lun': 1,
      'martes': 2, 'mar': 2,
...
app.post("/corregir-fechas-por-dia", async (req, res) => {
  try {
     ...
     */
    function encontrarFechaDelDia(diaTargetIndex: number, fechaAproximada: string): string {
...
app.post("/migrar-fechas-a-strings", async (req, res) => {
  try {
    ...
    function convertirTimestampAStringLocal(timestamp: any): string | null {
